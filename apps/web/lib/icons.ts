...
const TITLE_EMOJI_RULES: { pattern: RegExp; emoji: string }[] = [
  ...
  { pattern: /\bpilates\b/i,                  emoji: '🧘' },
  { pattern: /\bdance|dancing\b/i,            emoji: '💃' },
  { pattern: /\bvolleyball\b/i,               emoji: '🏐' },
  { pattern: /\bbasketball\b/i,               emoji: '🏀' },
  { pattern: /\bsoccer\b/i,                   emoji: '⚽' },
  { pattern: /\bfootball\b/i,                 emoji: '🏈' },
  { pattern: /\btennis\b/i,                   emoji: '🎾' },
  { pattern: /\bpickleball\b/i,               emoji: '🏓' },
  { pattern: /\bbaseball|softball\b/i,        emoji: '⚾' },
  { pattern: /\bgolf\b/i,                     emoji: '⛳' },
  { pattern: /\bhockey\b/i,                   emoji: '🏒' },
  { pattern: /\bbowling\b/i,                  emoji: '🎳' },
  { pattern: /\bswim(ming)?\b/i,              emoji: '🏊' },
  { pattern: /\bskat(e|ing)\b/i,              emoji: '⛸' },
  { pattern: /\bski(ing)?\b/i,                emoji: '⛷' },
  { pattern: /\bsnowboard(ing)?\b/i,          emoji: '🏂' },
  { pattern: /\bclimb(ing)?\b/i,              emoji: '🧗' },
  { pattern: /\bmartial\s*arts|karate|judo|taekwondo|jiu[- ]?jitsu\b/i, emoji: '🥋' },
  { pattern: /\bworkout|gym\b|\bfitness\b/i,  emoji: '🏋' },
  { pattern: /\bclinic|skills\b/i,            emoji: '🏅' },
...
export function placeholderFor(input: {
  ...
  // Walk rules in declared order, testing each one against title + venue
  // + organizer name combined. The rule order itself controls priority
  // (sport names are declared before activity-type names like "clinic"
  // or "workshop"), so a Luxe Volleyball Academy "Skills Clinic" picks
  // up 🏐 from the organizer name before the generic /clinic|skills/
